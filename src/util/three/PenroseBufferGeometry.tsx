    ...
    constructor(radius: number, inflations: number) {
        ...
        for (let j = 0; j < 5; j++) {
            triangles.push(
                new BTileS(
                    new Vector(
                        Math.cos(increment * j),
                        Math.sin(increment * j)
                    ).times(radius),
                    new Vector(0, 0),
                    new Vector(
                        Math.cos(increment * (j + 1)),
                        Math.sin(increment * (j + 1))
                    ).times(radius)
                )
            )
        }
        ...
        triangles = triangles.reduce(
            (result: Array<Triangle>, triangle: Triangle): Array<Triangle> => {
        ...
        this.setAttribute(
            'color',
            new THREE.BufferAttribute(new Float32Array(colors), colorComponents)
        )
    }
}
