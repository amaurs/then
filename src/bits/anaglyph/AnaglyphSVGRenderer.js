...
import {
    Projector,
    RenderableLine,
} from 'three/examples/jsm/renderers/Projector.js'
...
class AnaglyphSVGRenderer {
    ...
    clear() {
        ...
        this._svg.style.backgroundColor = this.getSvgColor(
            this._clearColor,
            this._clearAlpha
        )
    }
    ...
    render(scene, camera) {
        ...
        let _renderDataL = this._projector.projectScene(
            scene,
    ...
    renderLine(v1, v2, element, material, container) {
        let path =
            'M' +
            this.convert(v1.positionScreen.x) +
            ',' +
            this.convert(v1.positionScreen.y) +
            'L' +
            this.convert(v2.positionScreen.x) +
            ',' +
            this.convert(v2.positionScreen.y)
    ...
    getPathNode(id) {
        if (this._svgPathPool[id] == null) {
            this._svgPathPool[id] = document.createElementNS(
                'http://www.w3.org/2000/svg',
                'path'
            )
            if (this._quality === 0) {
                this._svgPathPool[id].setAttribute(
                    'shape-rendering',
                    'crispEdges'
                ) //optimizeSpeed
            }
