...
const Photography = (props: Props) => {
    ...
    const horizontalContainerRef = useRef<HTMLDivElement>(
        document.createElement('div')
    )
    const verticalContainerRef = useRef<HTMLDivElement>(
        document.createElement('div')
    )
    ...
    useEffect(() => {
        if (!presenting && props.data !== undefined) {
            const setOffsetHander = () => {
                ...
                let dynamicHeight =
                    objectWidth - window.innerWidth + window.innerHeight
    ...
    const createRow = (images: Array<Image>) => {
        return images.map((image, index) => (
            <div
                ...
                style={{
                    height: (props.height * 0.75) / props.rows,
                    width: props.width,
                }}
                key={index}
            >
    ...
    let rows = props.data.map((row, index) => (
        <div className="Photography-row" key={index}>
            {createRow(row)}
        </div>
    ))
    ...
    if (rows.length === 0 && presenting) {
        ...
        return (
            <div className="Photography" style={{ ...props.style, ...style }}>
                <div className="Photography-sticky" ref={verticalContainerRef}>
                    <div
                        className="Photography-horizontal"
                        style={{
